...
def random_hill_climb(problem, max_attempts=10, max_iters=np.inf, restarts=0,
                      ...
                      state_fitness_callback=None, callback_user_info=None, return_results=False):
    ...
    if max_iters is not np.inf:
        run_times = np.zeros(shape=(max_iters+1, max_attempts+1))
        call_tracker = np.zeros(shape=(max_iters+1, max_attempts+1))
    ...
    for current_restart in range(restarts + 1):
        ...
        while (attempts < max_attempts) and (iters < max_iters):
            ...
            if curve:
                ...
                if max_iters is not np.inf:
                    call_tracker[iters, current_restart] = attempts
                    run_times[iters, current_restart] = elapsed_time
    ...
    if max_iters is not np.inf:
        results = {"call_tracker": call_tracker[1:, :], "run_times": run_times[1:, :],
                   "avg_runtime": run_times.mean(axis=1)[1:]}
    if not return_results:
        ...
        if curve:
            if max_iters is not np.inf:
                return best_state, best_fitness, np.asarray(best_fitness_curve), results
            else:
                return best_state, best_fitness, np.asarray(best_fitness_curve)
