...
def mimic(problem, pop_size=200, keep_pct=0.2, max_attempts=10,
          ...
          state_fitness_callback=None, callback_user_info=None, noise=0.0, return_results=False):
    ...
    while (attempts < max_attempts) and (iters < max_iters):
        ...
        if curve:
            ...
            if max_iters is not np.inf:
                call_tracker[iters, attempts] = problem.get_adjusted_fitness()
                run_times[iters] = elapsed_time
    ...
    if max_iters is not np.inf:
        results = {"call_tracker": call_tracker, "run_times": run_times}
    if not return_results:
        ...
        if curve:
            if max_iters is not np.inf:
                return best_state, best_fitness, np.asarray(fitness_curve), results
            else:
                return best_state, best_fitness, np.asarray(fitness_curve)
