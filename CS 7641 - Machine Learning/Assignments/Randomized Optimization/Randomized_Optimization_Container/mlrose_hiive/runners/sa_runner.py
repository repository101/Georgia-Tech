...
class SARunner(_RunnerBase):
    ...
    def __init__(self, problem, experiment_name, seed, iteration_list, temperature_list, decay_list=None,
                 max_attempts=500, generate_curves=True, decay_pcts=None, **kwargs):
        ...
        if decay_pcts is None:
            decay_pcts = [0.99]
        self.decay_pcts = decay_pcts

    def run(self):
        temperatures = [decay(init_temp=t, decay=pct) for t in self.temperature_list for pct in self.decay_pcts for decay in self.decay_list]
