...
def render_policy_net(model, n_max_steps=200, seed=42):
    ...
    np.random.seed(seed)
    obs = env.reset()
    print("Hey we made it to Render Policy Net..\n")
    for step in range(n_max_steps):
        ...
        if done:
            break
    env.close()
    print("We are leaving Render Policy Net")
    return frames
...
for ax, width, history in zip(axes, (50, testing_iterations), (history1, history2)):
...
limited_repeating_env = suite_gym.load(
    "Breakout-v4",
    gym_env_wrappers=[partial(TimeLimit, max_episode_steps=testing_iterations)],
...
env.seed(42)
env.reset()
time_step = env.step(np.asarray([1])) # FIRE
for _ in range(4):
    time_step = env.step(np.asarray([3])) # LEFT
...
train_agent(n_iterations=testing_iterations)
...
def reset_and_fire_on_life_lost(trajectory):
    ...
    if prev_lives != lives:
        tf_env.reset()
        tf_env.pyenv.envs[0].step(np.asarray([1]))
        prev_lives = lives
