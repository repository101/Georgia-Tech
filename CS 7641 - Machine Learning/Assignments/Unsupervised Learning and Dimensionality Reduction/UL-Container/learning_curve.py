...
class LearningCurve(ModelVisualizer):
    ...
    def fit(self, X, y=None):
        ...
        curve = sk_learning_curve(self.estimator, X, y, return_times=True, **sklc_kwargs)
        self.train_sizes_, self.train_scores_, self.test_scores_, self.fit_times, self.predict_times = curve

        # compute the mean and standard deviation of the training times
        self.fit_times_mean_ = np.mean(self.fit_times, axis=1)
        self.fit_times_std_ = np.std(self.fit_times, axis=1)

        # compute the mean and standard deviation of the training times
        self.predict_times_mean_ = np.mean(self.predict_times, axis=1)
        self.predict_times_std_ = np.std(self.predict_times, axis=1)
