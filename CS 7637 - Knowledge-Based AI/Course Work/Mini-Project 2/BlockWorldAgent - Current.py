...
class State:
	def __init__(self, current_state, goal_state, goal_locations_by_name, goal_locations_by_index,
		...
		if non_moveable_blocks is not None:
	...
	def find_non_moveable(self):
		...
		for i in range(len(self.current_state)):
			for j in range(len(self.current_state[i])):
				if self.non_moveable_blocks is not None and self.current_state[i][j] not in self.non_moveable_blocks:
	...
	def get_current_state_locations(self):
		...
		for i in range(len(self.current_state)):
			if len(self.current_state[i]) > 0:
				if self.non_moveable_blocks is not None and self.current_state[i][-1] not in self.non_moveable_blocks:
	...
	def generate_next_states(self, some_state):
		...
		if len(some_state.moveable_blocks) > 0:
			for moveable_block in some_state.moveable_blocks:
				if moveable_block not in some_state.non_moveable_blocks:
					...
					if block_location[1] != 0:
						...
						if tuple_arrangement not in self.visited_states:
							...
							if self.goal_locations_by_name[current_block][1] == 0:
								temp_state.non_moveable_blocks.add(current_block)
							...
							possible_states.add(temp_state)
	...
	def solve(self, initial_arrangement, goal_arrangement):
		...
		while not is_solved:
			...
			while len(next_states) > 0:
				temp_state = next_states.pop()
				self.state_queue.put((temp_state.delta, temp_state))
				if temp_state.current_state not in self.visited_states:
					temp_queue.put((temp_state.delta, temp_state))
				if temp_state.determine_if_goal_state() or temp_state.delta == 0:
					is_solved = True
