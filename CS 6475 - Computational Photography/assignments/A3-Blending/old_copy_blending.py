...
def reduce_layer(image, kernel=generatingKernel(0.4)):
    ...
    try:
        ...
        try:
            ...
            reduced_image = convolved_image[0::2, 0::2]
            ...
            try:
                ...
                if (result_rows != reduced_rows) or (result_cols != reduced_cols):
                    reduced_image = reduced_image[0:int(reduced_rows), 0:int(reduced_cols)]
            except Exception as ResizeException:
                print("Exception when attempting to resize the reduced_image in reduce_layer.\n", ResizeException)
            result_rows, result_cols = reduced_image.shape
            if TESTING:
                if REDUCE:
                    cv2.imwrite("Reduce_Layer_{}_Original_Shape_{}_{}_rand_{}.jpg".
                                format(seed, image.shape[0], image.shape[1],
                                       np.random.randint(0, 20)), image)
                    cv2.imwrite("Reduce_Layer_{}_Reduced_Image_Shape_{}_{}_rand_{}.jpg".
                                format(seed, reduced_image.shape[0], reduced_image.shape[1],
                                       np.random.randint(0, 20)), reduced_image)
                    cv2.imwrite("Reduce_Layer_{}_Convolved_Imag_Shape_{}_{}_rand_{}.jpg".
                                format(seed, convolved_image.shape[0], convolved_image.shape[1],
...
def expand_layer(image, kernel=generatingKernel(0.4)):
    ...
    try:
        ...
        try:
            ...
            expanded_layer[0:expanded_rows:2, 0:expanded_cols:2] = image
            ...
            if TESTING:
                if EXPAND:
                    ...
                    cv2.imwrite("Expand_Layer_{}_Expanded_Image_Shape_Before_Convolve_{}_{}_rand_{}.jpg".
                                format(seed, expanded_before_convolve.shape[0], expanded_before_convolve.shape[1],
                    ...
                    cv2.imwrite("Expand_Layer_{}_Convolved_Image_After_Multiply_Shape_{}_{}_rand_{}.jpg".
                                format(seed, expanded_layer.shape[0], expanded_layer.shape[1],
                                       np.random.randint(0, 20)), expanded_layer)
                    cv2.imwrite("Expand_Layer_{}_Convolved_Image_Before_Multiply_Shape_{}_{}_rand_{}.jpg".
                                format(seed, expanded_layer_before_multiple.shape[0], expanded_layer_before_multiple.shape[1],
                                       np.random.randint(0, 20)), expanded_layer_before_multiple)
...
def gaussPyramid(image, levels):
    ...
    try:
        ...
        for gauss_level in range(levels):
            if gauss_level == 0:
                gaussPyramidResults.append(reduce_layer(gaussPyramidResults[0]).astype(dtype=np.float))
            else:
                gaussPyramidResults.append(reduce_layer(gaussPyramidResults[gauss_level]).astype(dtype=np.float))
        if TESTING:
            if GAUSS:
                cv2.imwrite("Gauss_Pyramid_Original_Image_{}{}.jpg".
                            format(image.shape[0], image.shape[1]), image)
                for g_level in range(len(gaussPyramidResults)):
                    cv2.imwrite("Gauss_Pyramid_Layer_{}_Image_{}_{}_rand_{}.jpg".
                                format(g_level, image.shape[0], image.shape[1],
                                       np.random.randint(0, 20)), image)
...
def laplPyramid(gaussPyr):
    ...
    try:
        ...
        for lapl_layer in range(len(gaussPyr)):
            ...
            gauss_height, gauss_width = gauss_layer.shape
            expand = expand_layer(gaussPyr[lapl_layer]).astype(dtype=np.float)
            try:
                if gauss_layer.shape != expand.shape:
                    expand = expand[0:gauss_height, 0:gauss_width]
            ...
            try:
                ...
                lapl_test_result = (expand - gauss_layer)
                lapl_results_list.append(lapl_result)
            except Exception as err:
                print("Exception calculating result in laplPyramid function. ", err)
            if TESTING:
                if LAPL:
                    try:
                        cv2.imwrite("Laplacian_Pyramid_Gauss.jpg".format(gauss_layer, gauss_layer.shape[0], gauss_layer.shape[1], np.random.randint(0, 20)), gauss_layer)
                        cv2.imwrite("Laplacian_Pyramid_Gauss_Layer_{}_Expanded_Image_{}_{}_rand_{}.jpg".
                                    format(gauss_layer, expand.shape[0], expand.shape[1],
                                           np.random.randint(0, 20)), expand)
                        cv2.imwrite("Laplacian_Pyramid_Gauss_Layer_{}_Result_Image_{}_{}_rand_{}.jpg".
                                    format(gauss_layer, lapl_result.shape[1], lapl_result.shape[1],
                                           np.random.randint(0, 20)), lapl_result)
                        cv2.imwrite("Laplacian_Pyramid_Gauss_Layer_{}_TEST_Result_Image_{}_{}_rand_{}.jpg".
                                    format(gauss_layer, lapl_test_result.shape[1], lapl_test_result.shape[1],
                                           np.random.randint(0, 20)), lapl_test_result)
        ...
        if lapl_results_list[0].dtype != np.float:
            raise TypeError
        if len(lapl_results_list) != len(gaussPyr):
            raise ValueError
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
    ...
    try:
        ...
        for layer in range(len(laplPyrBlack)):
            ...
            try:
                mask_before = gaussPyrMask[layer]
                ...
                print("Exception when manipulating the masks in blend function. \n", MaskException)
            # Apply mask
            try:
            ...
            except Exception as ApplyMaskException:
                print("Exception when attempting to apply the mask in blend function. \n", ApplyMaskException)
...
def collapse(pyramid):
    ...
    # WRITE YOUR CODE HERE.
    try:
        for layer in range(len(pyramid)-1, 0, -1):
            try:
            ...
            except Exception as CollapseSetupException:
                print("Exception when attempting to setup the base and next layer in the collapse function.\n", CollapseSetupException)
            try:
                expanded_base = expand_layer(pyramid[layer])
                if expanded_base.shape != next_layer.shape:
                    expanded_base = expanded_base[:int(next_layer_rows), :int(next_layer_cols)]
                combined_layer = np.add(next_layer, expanded_base)
            except Exception as CollapseExpandSectionException:
                print("Exception inside Collapse function near the expand. \n", CollapseExpandSectionException)
            if TESTING:
                if COLLAPSE:
                    try:
        ...
        if result_of_collapse.shape != pyramid[0].shape:
            raise ValueError
