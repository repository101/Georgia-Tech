...
def linearWeight(pixel_value):
    try:
        ...
        if type(pixel_value) == np.array or type(pixel_value) == np.ndarray:
            ...
            case_1 = np.where(pixel_value <= case_value)
            result_1 = pixel_value[case_1] - z_min
            case_2 = np.where(pixel_value > case_value)
            result_2 = z_max - pixel_value[case_2]
            pixel_value[case_1] = result_1
            pixel_value[case_2] = result_2
            return pixel_value
        else:
...
def sampleIntensities(images):
    try:
        ...
        for Zi in range(0, 256):  # +1 so inclusive of max
            locations = np.argwhere(mid_img == Zi)
            location_length = len(locations)
            if locations.size == 0:
                # Do Nothing
                continue
            else:
                random_xy = np.random.randint(0, location_length)
                temp = locations[random_xy]
                row = temp[0]  # Use as Y
                column = temp[1]  # Use as X
                for img_number in range(num_images):
                    intensity_values[Zi, img_number] = images[img_number][row, column]
        if intensity_values.dtype != np.uint8:
            intensity_values = np.uint8(intensity_values)
        return intensity_values

    except Exception as sampleIntensitiesException:
        print("Exception occurred while executing function 'sampleIntensities'.\n", sampleIntensitiesException)
...
def computeResponseCurve(intensity_samples, log_exposures, smoothing_lambda, weighting_function):
    try:
        ...
        # Following Matlab example from paper by Debevec and Malik
        k = 1
        ...
        for i in range(num_samples):
            for j in range(num_images):
                ...
                mat_A[i, (num_samples + i)] = -Wij
                #
                #   iii. Set mat_b at row k to Wij * log_exposure[j]
                mat_b[i] = Wij * log_exposures[j]
                k += 1
        #
        ...
        for Zk in range(z_min + 1, intensity_range - 1):
        ...
        #
        #     i. Set the value of mat_A in the last row and
        ...
        return g[:, 0]
    except Exception as computeResponseCurveException:
        print("Exception occurred while executing function 'computeResponseCurve'. \n", computeResponseCurveException)


def computeRadianceMap(images, log_exposure_times, response_curve, weighting_function):
    try:
        def getRadianceValue(pixel_array):
            # Defined inside computeRadianceMap because I need access to log_exposure_time and in this case
            #   I am not able to pass it in
            #        average radiance (i.e., sum(Wij * (g(Zij) - ln(tj))) / SumW),
            response_Zij = weighting_function(response_curve[pixel_array]) - log_exposure_times
            result = np.sum(pixel_array * response_Zij) / np.sum(pixel_array)
            return result
        ...
        if USE_FAST_IMPLEMENTATION:
            test_img_rad_map = img_rad_map.copy()
            images_D_Stack = np.dstack(np.asarray(images))
            Wij = np.apply_along_axis(weighting_function, 2, images_D_Stack)
            Sum_Wij = np.sum(Wij, axis=2)
            GreaterThanZero = np.apply_along_axis(getRadianceValue, 2, Wij.astype(np.uint8))
            middle_image = images[images_D_Stack.shape[-1]//2]
            test_img_rad_map[np.where(Sum_Wij > 0)] = GreaterThanZero[np.where(Sum_Wij > 0)]
            test_img_rad_map[np.where(Sum_Wij <= 0)] = response_curve[middle_image[np.where(Sum_Wij <= 0)]] - log_exposure_times[len(images) // 2]
            return test_img_rad_map
        for i in range(img_shape[0]):
            for j in range(img_shape[1]):
                Zij = [img[i, j] for img in images]
                Wij = [weighting_function(v) for v in Zij]
                SumW = np.sum(Wij)
                if SumW > 0:
                    img_rad_map[i, j] = np.sum(Wij * (response_curve[Zij] - log_exposure_times)) / SumW
                else:
                    img_rad_map[i, j] = response_curve[Zij[len(images) // 2]] - log_exposure_times[len(images) // 2]
        return img_rad_map
    except Exception as computeRadianceMapException:
        print("Exception occurred while executing function 'computeRadianceMap'.\n", computeRadianceMapException)
