...
def modelPrediction( data,players,pitcher = "Justin Verlander",
    ...
    # batter = players[players['MLBNAME'] == batter]['MLBID'].iloc[0]
    ...
    bookmark +=6
    record[bookmark+ all_pitches.index(pitchlag1)] = 1

    bookmark +=len(pitches)
    record[bookmark+ all_pitches.index(pitchlag2)] = 1

    bookmark +=len(pitches)
    record[bookmark+ all_pitches.index(pitchlag3)] = 1
    ...
    return dict(zip(pitches, np.round(loaded_model.predict(dtest)[0],2)))
...
def pitch_types_4_pitcher(pitcher_id, data):
    return list(data[data['pitcher'].isin([pitcher_id])].pitch_type.unique())
