...
def Manual_Strategy(symbol='JPM', sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2010, 6, 1),
	...
	RelativeStrengthIndex = Relative_Strength_Index(PricesDataFrame, lookback=14)
	...
	test = RelativeStrengthIndex < 40
	orders.ix[:] = np.NaN
	PricesDataFrame["Orders"] = np.zeros(shape=len(PricesDataFrame['Adj Close']), dtype=object)
	print ""
	test = PricesDataFrame.loc[(PricesDataFrame["Simple Moving Average Ratio"] < 0.98)]

	# mask = SimpleMovingAverageRatio.where(SimpleMovingAverageRatio < 0.98)
	# orders[(SimpleMovingAverageRatio < 0.98) & (BollingerBandPercentage < 0.3) & (RelativeStrengthIndex.loc[RelativeStrengthIndex < 40])] = tradeAmount
	test2 = PricesDataFrame.loc[(PricesDataFrame["Simple Moving Average Ratio"] > 1.02)]
