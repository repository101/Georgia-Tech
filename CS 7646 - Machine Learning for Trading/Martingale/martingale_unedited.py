...
def roulette_simulation(bankroll=None, episode_win=None, win_chance=0.6):
    ...
    if bankroll is not None:
        while total_winnings[0, count] > bankroll:
            won = False
            bet_amount = 1
            if count >= 999:
                break
            ...
            while not won:
                ...
                if won:
                    ...
                    total_winnings[0, count] = total_winnings[0, count - 1] + episode_winnings
                    bet_amount *= 2
                    if (total_winnings[0, count] + np.absolute(bankroll)) < bet_amount:
                        bet_amount = total_winnings[0, count]
