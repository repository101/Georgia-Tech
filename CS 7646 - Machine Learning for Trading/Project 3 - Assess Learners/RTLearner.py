...
class RTLearner(DTLearner.DTLearner):
    def __init__(self, leaf_size=1, verbose=False, **kwargs):
        ...
        if len(kwargs) >= 1:
            super().__init__(random_tree=True, **kwargs)
        else:
            super().__init__(leaf_size=self.leaf_size, verbose=self.verbose, random_tree=True)
