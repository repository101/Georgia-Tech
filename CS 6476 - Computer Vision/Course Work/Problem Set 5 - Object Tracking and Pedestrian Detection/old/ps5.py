	...
	def resample_particles(self):
		...
		if np.isnan(self.weights).any():
			if np.isnan(np.sum(self.weights)):
				...
				self.weights[np.where(np.isnan(self.weights))]
		try:
...
class MDParticleFilter(AppearanceModelPF):
	...
	def __init__(self, frame, template, is_part_5=False, beta=0.001, initial_momentum=0, **kwargs):
	...
	def augment_weight_wrt_direction(self):
		if self.direction is not None:
			if self.direction == "right":
				# loc = self.particles[(self.particles[:, 0] > self.previous_x_mean) & (self.particles[:, 1] > (self.current_y_mean - (self.template.shape[0] * 0.5))) & (self.particles[:, 1] < (self.current_y_mean + (self.template.shape[0] * 0.5)))]
				# loc = self.particles[(self.particles[:, 0] > self.previous_x_mean) & (
	...
	def process(self, frame, resize_template=True):
		...
		self.calc_direction()
		if self.direction is not None:
			# self.show_weights(wait=1)
			# self.show_particles(wait=0)
			self.augment_weight_wrt_direction()
			# self.show_particles(wait=1)
			# self.show_weights(wait=0)
		...
		if self.current_std_x < self.std_limit and self.current_std_y < self.std_limit:
			# cords = np.round(self.particles[np.argmax(self.weights), :], 0)
			...
			self.previous_alpha = self.alpha
			self.previous_template = np.copy(self.template)
			# max_weight = np.argmax(temp_weights)
			# cords = np.round(self.particles[max_weight, :], 1)
			temp_frame_cutout = self.validate_and_resize_template(
				frame[min_y_cords:max_y_cords, min_x_cords:max_x_cords])
			...
			if self.previous_sum_square_error_diff is not None:
				if (self.sum_square_error_diff - self.previous_sum_square_error_diff) < 0:
					...
					# temp_template = temp_frame_cutout
